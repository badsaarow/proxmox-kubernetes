...
USER=terraform-prov
PASSWD="gaam123$$%"
ROOT_SSH_KEY_FILE=./terraform/root_rsa
TERRAFORM_SSH_KEY_FILE=./terraform/id_rsa
...
proxmox-apt-install:
	ansible-playbook -i ../ansible-playbook/inventory-proxmox.yaml ../ansible-playbook/proxmox-apt-install.yaml -u root --private-key $(ROOT_SSH_KEY_FILE)

## fail
.PHONY: proxmox-install-docker
proxmox-install-docker:
	ansible-playbook -i ../ansible-playbook/inventory-proxmox.yaml ../ansible-playbook/proxmox-install-docker.yaml -u root  --private-key $(ROOT_SSH_KEY_FILE)

.PHONY: proxmox-add-admin-group
proxmox-add-admin-group:
	ansible-playbook -i ../ansible-playbook/inventory-proxmox.yaml ../ansible-playbook/proxmox-create-group.yaml -u root  --private-key $(ROOT_SSH_KEY_FILE)

.PHONY: proxmox-install-zerotier
proxmox-install-zerotier:
	ansible-playbook -i ../ansible-playbook/inventory-proxmox.yaml ../ansible-playbook/proxmox-install-zerotier.yaml -u root  --private-key $(ROOT_SSH_KEY_FILE)
...
&& qm importdisk 9002 jammy-server-cloudimg-amd64.img local --format qcow2 \
&& qm set 9002 --scsihw virtio-scsi-pci --scsi0 local:9002/vm-9002-disk-0.qcow2 \
